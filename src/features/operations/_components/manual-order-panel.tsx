...
// ─── Thin wrapper — keeps queries unmounted until sheet is open ───────────────
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button
          variant='default'
          className='shadow-md transition-shadow duration-300 hover:shadow-primary/10'
        >
          <Plus className='mr-2 h-4 w-4' />
          New Order
        </Button>
      </SheetTrigger>
      <SheetContent className='flex w-full flex-col gap-0 border-l-0 p-0 shadow-xl sm:max-w-2xl md:max-w-4xl lg:max-w-275'>
        {open && <ManualOrderContent onClose={() => setOpen(false)} />}
      </SheetContent>
    </Sheet>
  )
}

// ─── Content — only mounts when sheet is open ─────────────────────────────────
function ManualOrderContent({ onClose }: { onClose: () => void }) {
  ...
  // Queries — only run while sheet is mounted
  ...
  const derivedDiscounts = useMemo(
    () =>
    ...
    [orderDiscounts, cartItems, totalAmount]
  )
  ...
  const totalPayable = useMemo(() => {
    ...
    return Math.max(0, totalAmount - totalDiscounts)
  }, [totalAmount, derivedDiscounts])

  const discountedItemTempId = useMemo(() => {
    const productScopeDiscount = derivedDiscounts.find(
      (d) => d.scope === 'PRODUCT'
    )
    if (!productScopeDiscount || cartItems.length === 0) return null
    const maxPrice = Math.max(...cartItems.map((i) => i.unitPrice))
    return cartItems.find((i) => i.unitPrice === maxPrice)?.tempId || null
  }, [derivedDiscounts, cartItems])

  // ── Handlers ──────────────────────────────────────────────────────────────
  const openProductSelector = useCallback((product: Product) => {
  ...
  const handleAddToCart = useCallback(() => {
    ...
    const newItem: CartItem = {
      ...
      options: { choiceId: selectedChoiceIds },
      displayOptions,
    }

    setCartItems((prev) => [...prev, newItem])
    setSelectedProduct(null)
  }, [selectedProduct, selectedVariantId, selectedModifiers])
  ...
  const handleAddDiscount = useCallback(
    (discount: { reason: string; amount: number; type: 'FIXED' | 'PERCENTAGE' }) => {
      const appliedAmount =
        discount.type === 'FIXED'
          ? discount.amount
          : Math.floor(totalAmount * (discount.amount / 100) * 100) / 100

      setOrderDiscounts([{ ...discount, id: crypto.randomUUID(), appliedAmount }])
  ...
  const handleApplyPromotion = useCallback(
    (promo: Promotion) => {
      ...
      setOrderDiscounts([{ ...discountData, id: crypto.randomUUID() }])
  ...
  const handleCommit = useCallback(() => {
    ...
    if (!method) {
      return toast.error(`Fulfillment method ${orderType} is currently unavailable`)
    }

    const topLevelDiscounts = derivedDiscounts.filter(
      (d) => d.scope === 'CART' || !d.scope
    )
    const productDiscounts = derivedDiscounts.filter((d) => d.scope === 'PRODUCT')
    ...
    createOrder(
      {
        ...
        items: cartItems.map(({ tempId, name, imageUrl, displayOptions, ...item }) => {
          const isDiscounted = tempId === discountedItemTempId
          return {
            ...item,
            itemDiscounts: isDiscounted
              ? productDiscounts.map(({ id, appliedAmount, scope, ...rest }) => rest)
              : undefined,
          }
        }),
    ...
    <>
      <SheetHeader className='bg-primary px-6 py-5 text-primary-foreground select-none'>
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-[10px] font-bold tracking-wider uppercase'>
      ...
      <div className='flex flex-1 overflow-hidden bg-[#F8F9FB]'>
        ...
        <div className='flex flex-1 flex-col overflow-hidden'>
          <div className='z-10 bg-white px-6 py-4 shadow-sm'>
            <div className='flex flex-wrap items-center justify-between gap-6'>
              ...
              <div className='min-w-75 flex-[1.5]'>
                    ...
                    {selectedCustomer ? (
                      ...
                      <div className='group relative'>
                        ...
                        <Input
                          ...
                          className='h-10 border pl-10 text-xs transition-colors focus-visible:border-primary focus-visible:ring-0'
                          value={searchQuery}
                  ...
                  <div className='grid animate-in grid-cols-2 gap-3 fade-in slide-in-from-top-1'>
                    <div className='relative'>
                      <Input
                        ...
                        className='h-10 border text-xs transition-colors focus-visible:border-primary focus-visible:ring-0'
                        value={customerName}
          ...
          <div className='flex flex-1 overflow-hidden'>
            <div className='w-48 overflow-y-auto border-r bg-white pt-4 pb-12 select-none'>
              ...
              <div className='space-y-0.5'>
                <button
                  className={cn(
                    'group relative flex w-full items-center justify-between overflow-hidden px-6 py-3.5 transition-colors',
              ...
              {isLoadingProducts ? (
                ...
                <div className='grid grid-cols-2 content-start gap-4 pb-20 lg:grid-cols-3 xl:grid-cols-4'>
                  {products.map((product: Product) => (
                    <Card
                      key={product.id}
                      className='group cursor-pointer overflow-hidden border-transparent bg-white shadow-sm transition-shadow duration-200 hover:border-primary/30 hover:shadow-md active:scale-[0.98]'
        ...
        {/* Cart Section */}
        <div className='z-20 flex w-85 flex-col border-l bg-white shadow-sm'>
            ...
            {cartItems.length === 0 ? (
              ...
              <div className='space-y-2.5'>
                {cartItems.map((item) => (
                  <div
                    key={item.tempId}
                    className='group flex items-center gap-3 rounded-lg border border-transparent bg-muted/20 p-2.5 transition-colors hover:border-muted-foreground/10'
                  >
                    ...
                    <div className='min-w-0 flex-1'>
                      <div className='mb-0.5 flex items-center justify-between'>
                        <div className='flex items-center gap-2 overflow-hidden'>
                          <span className='max-w-35 truncate text-[11px] font-bold text-foreground'>
                            {item.name}
                          </span>
                    ...
                    <button
                      className='h-6 w-6 rounded-full text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:text-destructive'
